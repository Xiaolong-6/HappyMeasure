...
def test_simulated_smu_driver_iv_cv_and_context(monkeypatch):
    ...
    with drv:
        ...
        drv.output_on()
        assert drv.output_enabled is True
        ...
        drv.configure_source_measure(
            SourceMode.VOLTAGE, MeasureMode.CAPACITANCE, compliance=1, nplc=1
        )
...
def test_measurement_service_runs_and_turns_output_off(monkeypatch):
    ...
    class FakeDriver:
        ...
        def configure_source_measure(self, **kwargs):
        ...
        def read(self):
            return DriverReadback(self.current, self.current * 2)
...
def test_measurement_service_is_ui_boundary_for_legacy_source_meters(monkeypatch):
    ...
    class FakeMeter:
        ...
        def configure_for_sweep(self, _config):
            pass
        ...
        def set_source(self, _mode, value):
            self.source = value

        def read_source_and_measure(self):
            return self.source, self.source * 2
    ...
    result = MeasurementService.run_source_meter(meter, config)
    assert [point.source_value for point in result.points] == [-1, 0, 1]
    assert meter.output_enabled is False
...
def test_keithley_driver_adapter_uses_serial_backend(monkeypatch):
    ...
    class FakeSerial:
        ...
        def identify(self):
            return "KEITHLEY,MODEL 2400,1,2"

        def reset(self):
            pass

        def configure_for_sweep(self, cfg):
            self.config = cfg

        def set_source(self, source_cmd, value):
            self.source = (source_cmd, value)
...
def test_serial_2400_with_fake_serial(monkeypatch):
    ...
    class FakeSerialModule:
        EIGHTBITS = FakePort.EIGHTBITS
        PARITY_NONE = FakePort.PARITY_NONE
        STOPBITS_ONE = FakePort.STOPBITS_ONE
    ...
    cfg = SweepConfig(
        mode=SweepMode.VOLTAGE_SOURCE, start=0, stop=0, step=1, compliance=0.01, nplc=1
    )
...
def test_formatting_helpers():
    from keith_ivt.utils.formatting import (
        format_current,
        format_resistance,
        format_si,
        format_voltage,
