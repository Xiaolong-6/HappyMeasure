...
def test_manual_region_has_bounded_drag_and_numeric_edit_path(application) -> None:
    ...
    page.mode_combo.setCurrentIndex(
        page.mode_combo.findData(DarkCorrectionMode.MANUAL_REGIONS)
    )
    page._add_region()
...
def test_invalid_gate_does_not_fall_back_to_previous_valid_preparation(application) -> None:
    ...
    page.mode_combo.setCurrentIndex(
        page.mode_combo.findData(DarkCorrectionMode.ROLLING_QUANTILE)
    )
...
def test_global_export_menu_retains_previous_export_capabilities(application) -> None:
    window = MapReconstructionWindow()
    labels = {
        action.text()
        for action in window.workflow_header.export_menu.actions()
        if action.text()
    }
    assert {
        "Prepared time trace",
