...
def ui_text() -> str:
    return "\n".join(
        path.read_text(encoding="utf-8") for path in (SRC / "keith_ivt" / "ui").glob("*.py")
    )
...
def test_single_bottom_operator_bar_replaces_content_local_controls():
    ...
    operator = operator_src[
        operator_src.index("def _build_operator_bar") : operator_src.index(
            "def _update_operator_layout"
        )
    ]
    assert 'self.action_bar = ttk.Frame(self.root, style="Operator.TFrame"' in operator
    compact_operator = "".join(operator.split())
    assert (
        'self.action_bar.grid(row=1,column=getattr(self,"_workspace_column",0)' in compact_operator
    )
...
def test_post5_interaction_hotfix_contracts():
    ...
    assert "suggested_single_csv_name" in source_text(
        "ui/trace_panel.py"
    ) and "suggested_all_csv_name" in source_text("ui/trace_panel.py")
    assert "zoom_x = bool(state & 0x0004)" in all_ui
    assert "zoom_y = not zoom_x" in all_ui
    assert (
        'pair[1].configure(state="disabled" if (is_auto or not editable) else "normal")' in all_ui
    )
...
def test_mockup_locked_core_flat_styles():
    ...
    assert 'background=self._palette["forest"],foreground="#FFFFFF"' in compact_styles
    assert 'background=self._palette["danger"],foreground="#FFFFFF"' in compact_styles
...
def test_statusbar_connection_summary_and_click_outside_drawer():
    ...
    assert (
        "self.header_status = ttk.Label(self.page_header, textvariable=self.status_connection_text"
        not in s
    )
    status = source_text("ui/status_bar.py")
    nav = source_text("ui/navigation.py")
...
def test_plot_toolbar_is_view_only_and_actions_are_context_menu():
    ...
    for text_label, menu_label in [
        ("Autorange", "Autorange this view"),
        ("Fullscreen", "Open fullscreen"),
        ("Save Plot", "Save plot image..."),
    ]:
...
def test_post6_animation_constant_and_split_contracts():
    ...
    assert (
        "HardwareControllerMixin" in mixins
        and "SweepControllerMixin" in mixins
        and "DataActionMixin" in mixins
    )
...
def test_alpha4_startup_splitter_restore_contracts():
    ...
    assert 'self.style.configure("Nordic.TPanedwindow"' in "".join(theme.split())
    ...
    restore = restore_src[
        restore_src.index("def _build_restore_panel") : restore_src.index("def refresh_backup_list")
    ]
...
def test_alpha6_plot_trace_splitter_uses_portable_panes_contract():
    ...
    helper = plot[
        plot.index("def _safe_add_plot_trace_pane") : plot.index("def _initialize_plot_trace_sash")
    ]
