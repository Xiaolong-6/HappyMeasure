"""Behavioral tests for display-only Time plot policies."""
...
def test_last_n_window_is_display_only_and_keeps_newest_points() -> None:
    x = list(range(10_000))
...
def test_history_window_handles_short_and_invalid_counts() -> None:
    assert time_display_window([1, 2, 3], [4, 5, 6], "Last N points", 99) == (
        [1, 2, 3],
        [4, 5, 6],
    )
    assert time_display_window(list(range(7)), list(range(7, 14)), "Last N points", 2) == (
        [5, 6],
        [12, 13],
    )
...
def test_all_data_trims_mismatched_source_lengths_safely() -> None:
    assert time_display_window([1, 2, 3], [10, 20], "All data", 1) == ([1, 2], [10, 20])
