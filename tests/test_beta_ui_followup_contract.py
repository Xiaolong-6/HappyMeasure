...
def test_status_bar_is_compact_and_uses_live_vi_readout() -> None:
    ...
    assert "Running {self.point_count}" in app_state
    assert "Paused ({self.point_count} points)" in app_state
    assert '"Src -- · Meas -- · Cmpl --"' in simple


def test_range_auto_buttons_remain_clickable_outside_active_runs() -> None:
    sweep_config = read("src/keith_ivt/ui/sweep_config.py")
    assert '{"idle", "stopped", "completed", "aborted"}' in sweep_config
    assert "editable = bool(self._connected and not busy)" in sweep_config
...
def test_trace_visible_column_shows_checked_and_latest_trace_selection_hook_exists() -> None:
    ...
    assert "self._selected_trace_id" in trace_panel
    assert "trace = self._datasets.add_result" in sweep_controller
    assert "self._selected_trace_id = trace.trace_id" in sweep_controller
...
def test_plot_context_menu_supports_xy_axis_swap() -> None:
    ...
    assert "def _swap_xy_for_axis_view" in panel
    assert "ax.set_xscale" in panel
