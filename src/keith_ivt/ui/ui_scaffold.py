...
class UiScaffoldMixin(UiMixinTyping):
    def _build_content_scaffold(self) -> None:
        self.content_frame.rowconfigure(0, weight=0)
        self.content_frame.rowconfigure(1, weight=1)
        self.content_frame.columnconfigure(0, weight=1)

        self.page_header = ttk.Frame(
            self.content_frame, style="Topbar.TFrame", padding=(12, 10, 12, 6)
        )
        ...
        try:
            add_tip(
                self.page_title,
                getattr(self, "NAV_TIPS", {}).get(self._active_nav, "Current workspace panel"),
            )
        except Exception:
            pass
        ...
        self.content_canvas = tk.Canvas(
            self.content_frame, highlightthickness=0, borderwidth=0, background=self._palette["bg"]
        )
        ...
        self._content_window_id = self.content_canvas.create_window(
            (0, 0), window=self.current_content, anchor="nw"
        )
    ...
    def _on_content_mousewheel(self, event) -> str | None:
        try:
            delta = (
                ...
                else (1 if getattr(event, "num", None) == 5 else -1)
            )
