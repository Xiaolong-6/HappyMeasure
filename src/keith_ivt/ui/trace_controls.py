...
class TraceInteractionMixin(UiMixinTyping):
    def _show_trace_context_menu(self, event) -> None:
        ...
        menu.add_command(
            label="Hide / show selected", command=self.toggle_selected_trace_visibility
        )
        menu.add_separator()
    ...
    def choose_trace_color(self, trace_id: int | None = None) -> None:
        ...
        _rgb, color = colorchooser.askcolor(
            color=getattr(trace, "color", "#1f77b4"), title=f"Choose color - {trace.name}"
        )
        if color:
