...
class PanelBuilderMixin(UiMixinTyping):

    def _available_ui_fonts(self) -> list[str]:
        """Return installed UI fonts with Verdana preferred when available."""
        try:
            fonts = sorted(
                {str(f) for f in tkfont.families(self.root) if str(f).strip()}, key=str.lower
            )
        except Exception:
            fonts = ["Verdana"]
        if "Verdana" in fonts:
    ...
    def _build_hardware_panel(self, parent) -> None:
        ...
        self.port_combo = self._combo(
            box,
            "COM port",
            self.port,
            self._refresh_port_choices(),
            0,
            "Detected serial ports. Debug mode exposes a simulated COM3.",
        )
        self.baud_combo = self._combo(
            box,
            "Baud",
            self.baud_rate,
            [9600, 19200, 38400, 57600],
            1,
            "Default RS-232 baud rate for Keithley 2400-class instruments.",
        )
        self.terminal_combo = self._combo(
            box,
            "Terminal",
            self.terminal,
            ["FRONT", "REAR"],
            2,
            "Front or rear terminal selection. Hidden/disabled later for devices without front/rear routing.",
        )
        self.sense_combo = self._combo(
            box,
            "Sense",
            self.sense_mode,
            ["2-wire", "4-wire"],
            3,
            "2-wire or 4-wire remote sense. Future drivers advertise whether 4-wire is available.",
        )
        ...
        add_tip(
            self.connect_btn,
            "Connect to the selected instrument, or disconnect the active connection when already connected.",
        )
        ...
        ttk.Label(
            cap_box,
            textvariable=self.hardware_profile_text,
            style="Card.TLabel",
            wraplength=380,
            justify="left",
    ...
    def _build_sweep_panel(self, parent) -> None:
        ...
        self.mode_combo = self._combo(
            ...
            0,
            "Capability-aware source mode. Current-source or voltage-source IV mode.",
        )
        self.sweep_kind_combo = self._combo(
            top,
            "2. Sweep type",
            self.sweep_kind,
            self._available_sweep_kinds(),
            1,
            "Sweep algorithm for IV testing. Use Time for fixed-value time traces.",
        )
        ...
        if self.debug.get():
            self.debug_model_row = self._combo(
                top,
                "3. Debug load model",
                self.debug_model,
                debug_model_names(),
                3,
                "Simulator-only load/response model. In voltage-source mode it returns current; in current-source mode it returns voltage.",
            )
        ...
        self._entry(
            ...
            self.compliance,
            0,
            "Compliance limit. Units depend on source mode.",
        )
        self._entry(
            self.common_box,
            "NPLC",
            self.nplc,
            1,
            "Power-line cycles per integration. Higher NPLC improves noise but increases minimum interval.",
        )
        self._entry(
            self.common_box,
            "Delay (s)",
            self.delay_s,
            2,
            "Per-point source-settling delay before readback. Default 0 s. Included in the time estimate and Keithley 2400 source delay command.",
        )
    ...
    def _build_settings_panel(self, parent) -> None:
        ...
        self._check(
            ...
            self.debug,
            0,
            "Keep enabled until simulator, UI, export, and import paths are stable.",
        )
        self._entry(
            box,
            "Log max KB",
            self.log_max_kb,
            1,
            "Rotating log size limit in KB. When logs/log.txt would exceed this limit, a new log file is created.",
        )
        try:
        ...
        self._entry(
            ...
            3,
            "Write temporary cache every N points when enabled.",
        )
        if self.debug.get():
            ttk.Label(box, text="Debug UI appearance", style="Muted.TLabel").grid(
            ...
            self.ui_font_combo = self._combo(
                ...
                5,
                "Application UI font family. This list is read from fonts installed on this system.",
            )
            ...
            self.ui_theme_combo = self._combo(
                ...
                ["Light", "Dark", "Debug"],
                7,
                "Light is the default clean theme. Debug keeps strong borders for layout inspection; Dark uses integrated dark backgrounds.",
            )
            ...
            ttk.Label(
                box,
                text="UI appearance controls are shown only in debug mode.",
                style="Muted.TLabel",
                wraplength=360,
                justify="left",
            ).grid(row=4, column=0, columnspan=2, sticky="ew", pady=(10, 0))
            save_row = 5
        ttk.Button(box, text="Default Settings...", command=self.review_and_save_settings).grid(
            row=save_row, column=0, columnspan=2, sticky="ew", pady=(10, 0)
        )
        restart_row = save_row + 1
        ttk.Button(box, text="Restart UI", command=self._restart_ui, style="Soft.TButton").grid(
            row=restart_row, column=0, columnspan=2, sticky="ew", pady=(6, 0)
        )
    ...
    def _restart_ui(self) -> None:
        ...
        if not messagebox.askyesno(
            "Restart UI", "Restart the HappyMeasure UI?\n\nAny unsaved changes will be lost."
        ):
            return
        try:
            ...
            messagebox.showerror(
                "Restart Failed", f"Failed to restart UI:\n{e}\n\nPlease restart manually."
            )
    ...
    def _build_log_panel(self, parent) -> None:
        ...
        ttk.Button(btns, text="Open log folder", command=self.open_log_folder).grid(
    ...
    def _build_about_panel(self, parent) -> None:
        ...
        ttk.Label(
            box,
            textvariable=self.update_notice_text,
            style="AboutStatus.TLabel",
            wraplength=450,
            justify="left",
        ...
        update_btn = ttk.Button(
            box,
            text="Open Latest Release Download Page",
        ...
        add_tip(
            update_btn,
            "Open the latest HappyMeasure release page if known; otherwise open the project repository.",
        )
        row += 1
        ttk.Label(
            box,
            text="What is HappyMeasure?",
            style="AboutTitle.TLabel",
            font=(
                getattr(self.settings, "ui_font_family", "Verdana"),
                int(getattr(self.settings, "ui_font_size", 10)),
        ...
        ttk.Label(
            box, text=description, style="AboutBody.TLabel", wraplength=450, justify="left"
        ...
        for feature in features:
            ttk.Label(box, text=feature, style="AboutBody.TLabel", wraplength=450).grid(
        ...
        ttk.Label(
            box, text=hardware, style="AboutBody.TLabel", wraplength=450, justify="left"
        ).grid(row=row, column=0, sticky="w", pady=(0, 12))
        row += 1
        ttk.Label(
            box,
        ...
        parent.bind(
            "<Configure>", lambda e: self._update_about_wraplength(scroll_frame, canvas), add="+"
        )
