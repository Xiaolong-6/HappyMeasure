...
class PresetRestorePanelMixin(UiMixinTyping):
    def _build_preset_panel(self, parent) -> None:
        ...
        for i, (text, cmd) in enumerate(
            [
                ("Refresh", self.refresh_preset_list),
                ("Load", self.load_selected_preset),
                ("Save As...", self.save_named_preset_dialog),
                ("Delete", self.delete_selected_preset),
            ]
        ):
            ttk.Button(btns, text=text, style="Soft.TButton", command=cmd).grid(
                row=0, column=i, sticky="ew", padx=3
            )

        # Treeview below buttons with weight=1 to fill remaining space
        self.preset_list = ttk.Treeview(
            box, columns=("name",), show="headings", selectmode="browse"
        )
    ...
    def _build_restore_panel(self, parent) -> None:
        ...
        ttk.Label(folder_row, text="Backup folder", style="Card.TLabel").grid(
            row=0, column=0, sticky="w", padx=(0, 6)
        )
        ttk.Entry(folder_row, textvariable=self.backup_folder_var).grid(
            row=0, column=1, sticky="ew"
        )
        ttk.Button(
            folder_row, text="Browse...", style="Soft.TButton", command=self.choose_backup_folder
        ...
        ttk.Button(
            btns, text="Refresh", style="Soft.TButton", command=self.refresh_backup_list
        ).grid(row=0, column=0, sticky="ew", padx=(0, 4), pady=(0, 4))
        ttk.Button(
            btns, text="Import", style="Soft.TButton", command=self.import_selected_backup
        ).grid(row=0, column=1, sticky="ew", padx=4, pady=(0, 4))
        ttk.Button(btns, text="Open", style="Soft.TButton", command=self.open_backup_folder).grid(
        ...
        ttk.Label(box, textvariable=self.backup_text, style="Muted.TLabel").grid(
