...
def build_keithley2400_sweep_command_plan(
    config: SweepConfig, *, include_output: bool = True
) -> list[str]:
    ...
    if config.auto_source_range:
        ...
        commands.extend(
            [f":SOUR:{src}:RANG:AUTO OFF", f":SOUR:{src}:RANG {config.source_range:.12g}"]
        )
    if config.auto_measure_range:
        commands.append(f":SENS:{meas}:RANG:AUTO ON")
    else:
        commands.extend(
            [f":SENS:{meas}:RANG:AUTO OFF", f":SENS:{meas}:RANG {config.measure_range:.12g}"]
        )
